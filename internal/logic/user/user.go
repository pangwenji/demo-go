...
func (s *sUser) Create(ctx context.Context, in model.UserCreateInput) (err error) {
	...
	if in.Nickname == "" {
		in.Nickname = in.Passpost
	}
	var (
		available bool
	)
	// Passport checks.
	available, err = s.IsPassportAvailable(ctx, in.Passpost)
	if err != nil {
		return err
	}
	if !available {
		return gerror.Newf(`Passport "%s" is already token by others`, in.Passpost)
	}
	// Nickname checks.
